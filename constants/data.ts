...
// Will be replaced with real API/Supabase calls
...
export const generateEquityCurve = (days: number, startBalance: number) => {
  const data = [];
  let balance = startBalance;
  for (let i = days; i >= 0; i--) {
    const change = balance * 0.02 * (Math.random() - 0.38);
    balance = Math.max(startBalance * 0.88, balance + change);
    data.push({
      value: parseFloat(balance.toFixed(2)),
      label: i % 7 === 0 ? `D${days - i}` : '',
    });
  }
  return data;
};
...
export const instrumentBreakdown = [
  { value: 38, color: '#5eead4', text: 'XAUUSD' },
  { value: 22, color: '#0d9488', text: 'EURUSD' },
  { value: 18, color: '#4dd9b8', text: 'NAS100' },
  { value: 12, color: '#fbbf24', text: 'US30' },
  { value: 10, color: '#7ab8ac', text: 'Other' },
];
